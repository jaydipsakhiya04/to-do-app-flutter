...
class ChatViewScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: ListTile(
          ...
          subtitle:
              CommonText(text: "online", fontSize: 10.sp, color: AppColors.middleGrey, fontWeight: FontWeight.w400),
          // trailing: Icon(Icons.more_vert_outlined, color: AppColors.titleDark),
          trailing: PopupMenuButton<String>(
            color: AppColors.darkBackGroundHome,

            icon: SvgPicture.asset(IconAsset.verticalMore,width: 25.w,height: 24.w,),
            ...
            itemBuilder: (BuildContext context) {
              return <PopupMenuEntry<String>>[
                 PopupMenuItem<String>(
                  value: "More",
                  child: Text('More',style: TextStyle(color: AppColors.whiteColor)),
                ),
                 PopupMenuItem<String>(
                  value: 'Cancel',
  ...
  Widget chatContainer(ChatDataModel chatDataModel, bool show) {
    return Column(
      crossAxisAlignment:
          chatDataModel.messageType == AppString.receiver ? CrossAxisAlignment.end : CrossAxisAlignment.start,
      children: [
        if (show)
          CommonText(
            text: chatDataModel.messageType == AppString.receiver ? AppString.me : AppString.resturantnameUpper,
            fontSize: 10.sp,
        ...
        Row(
          mainAxisSize: MainAxisSize.min,
          mainAxisAlignment:
              chatDataModel.messageType == AppString.receiver ? MainAxisAlignment.end : MainAxisAlignment.start,
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                color:
                    (chatDataModel.messageType == AppString.receiver ? AppColors.purple : AppColors.darkBackGroundHome),
              ),
              child: Text(
                chatDataModel.msg ?? "",
                style: TextStyle(
  ...
  Widget chatList() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (index != 0) {
          show.value = _chatDataController.messageList[index].messageType !=
              _chatDataController.messageList[index - 1].messageType;
        }

        return GestureDetector(
          child: chatContainer(_chatDataController.messageList[index], show.value),
        );
      },
    );
  }
            ...
            Expanded(
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  hintStyle: TextStyle(color: AppColors.bodyDark, fontSize: 14.sp, fontWeight: FontWeight.w500),
                ),
              ),
            ),
            Obx(() => _chatDataController.change.value ? SvgPicture.asset(IconAsset.attachFile) : SizedBox()),
            Obx(
              () => _chatDataController.change.value
                  ? Container(
                      width: 1,
